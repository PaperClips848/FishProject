...
fileName_c = '../data/seth_environmentalData_march2023.csv';    % file name containing data
ed_t = readtable(fileName_c);                           % reading the csv into a table

sight = 15;

ed_m = ed_t{:, 3:end};                                  % converting the table into a matrix
...
bar(edl_c, ed_v);                                      % bar graph of the number of fish in each genus
title(['Sight #', num2str(sight, '%11.1f')]);
xlabel('Attribute');                                         % labeling the x-axis
